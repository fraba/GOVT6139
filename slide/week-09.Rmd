...
- Used when:
  ...
  - Probability of success ( $θ$ ) is **constant** (probability describes the underlying model for generating the data, in this case a dice)
...
**Example**: Rolling 20 dice, counting "💀"
- $N$ = 20 trials
- $θ$ = 1/6 (probability of "💀")
...
chart2 <- ggplot(sim_data, aes(x = skulls)) +
  ...
  annotate("text", x = 11, y = max(theoretical_prob) * 0.9,
           label = sprintf("10,000 simulations\nMean: %.2f\nSD: %.2f", 
                          mean(skulls), sd(skulls)),
           color = "steelblue", fontface = "bold", size = 3,
           hjust = 0) +
  labs(title = "Observed Results from Simulations",
...
split_point <- 10  # Split at 10 skulls

# First half (0-9 skulls)
table1 <- comparison_table[comparison_table$Number.of.Skulls <= (split_point - 1), ]

# Second half (10-20 skulls)
table2 <- comparison_table[comparison_table$Number.of.Skulls >= split_point, ]
...
**Example**: Does ESP (Extrasensory Perception) exist?
