---
title: "GOVT6139</br>Research design"
subtitle: "Week 10</br>Quantitative Methods: Data Analysis Lab (with JASP)"
author: "Francesco Bailo"
institute: "The University of Sydney"
date: "Semester 1, 2024 (updated: `r Sys.Date()`)"
output:
  xaringan::moon_reader:
    css: ["default", "assets/sydney-fonts.css", "assets/sydney.css"]
    self_contained: false # if true, fonts will be stored locally
    seal: true # show a title slide with YAML information
    includes:
      in_header: "assets/mathjax-equation-numbers.html"
    nature:
      beforeInit: ["assets/remark-zoom.js", "https://platform.twitter.com/widgets.js"]
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
      ratio: '16:9' # alternatives '16:9' or '4:3' or others e.g. 13:9
      navigation:
        scroll: false # disable slide transitions by scrolling

---

background-image: url(https://upload.wikimedia.org/wikipedia/en/6/6a/Logo_of_the_University_of_Sydney.svg)
background-size: 95%

```{r setup, include=FALSE}

knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, 
                      dev = 'svg', out.width = "45%", fig.width = 6,
                      fig.align="center")

options(scipen = 999)
```

---

## Acknowledgement of Country

I would like to acknowledge the Traditional Owners of Australia and  recognise their continuing connection to land, water and culture. The  University of Sydney is located on the land of the Gadigal people  of the Eora Nation. I pay my respects to their Elders, past and present.


---


## Course outline

.pull-left[

**Week 1**: Introduction

**PART I PRELIMINARY CONSIDERATIONS (Weeks 2-5)**

**Week 2**: The Selection of a Research Approach + Guest lecture w/t Inessa De Angelis

**Week 3**: Review of the Literature

**Week 4**: The Use of Theory + Guest lecture w/t Assel Mussagulova

**Week 5**: Writing Strategies and Ethical Considerations

**PART II DESIGNING RESEARCH (Weeks 6-12)**

**Week 6**: The Introduction


]

.pull-right[


**Week 7**: The Purpose Statement

**Week 8**: Research Questions and Hypotheses + Guest Lecture w/t Minglu Chen

⏸️ *Mid Semester break* ⏸️

**Week 9**: Quantitative Methods  👈

**Week 10**: Quantitative Methods: Data Analysis Lab (w/t JASP)

**Week 11**: Qualitative Methods + Guest lecture w/t Sarah Phillips

**Week 12**: Qualitative Methods: Data Analysis Lab (Make sure you install NVivo)

**Week 13**: Conclusions


]


---

class: inverse, center, middle

# Quantitative Methods: Data Analysis Lab (with JASP)


# Getting Started with JASP
## Introduction & Descriptive Statistics

### A Hands-On Guide with Titanic Dataset

---

## JASP Interface Overview

### What You See When You Start JASP

**Main Components:**

.pull-left[

1. **Top Menu Bar**
   - File, Edit, Data tabs
   - Analysis buttons (Descriptives, T-Tests, Regression, etc.)

2. **Left Panel: Data View**
   - Spreadsheet with rows (cases) and columns (variables)
   - Each row = one participant/observation
   - Each column = one variable

]

.pull-right[

3\. **Right Panel: Results**
   - Analysis output appears here
   - Updates in real-time
   - Can copy/export results

4\. **Analysis Options Panel** (center-left when analysis selected)
   - Choose variables
   - Select options
   - Customize analysis

]



---

## Loading Data into JASP

### Two Main Ways to Get Data

.pull-left[

**Method 1: Open CSV Files (Most Common)**
1. Click **File** → **Open**
2. Select **Computer** → **Browse**
3. Navigate to your `.csv` file (you can also open Excel files)
4. Click **Open**

**Method 2: Open .jasp Files**
- Contains both data AND previous analyses
- Click **File** → **Open** → Select `.jasp` file
- (This is also the format for your to save your JASP workspace)
]

.pull-right[


**CSV File Format:**
- Plain text file
- Comma-separated values
- Can be created in Excel, Google Sheets, etc.
- First row = variable names

**First Dataset:** `titanic.csv`

]



---

## Understanding Variable Types

### Three Variable Types in JASP

JASP uses **symbols** in column headers to show variable type:

| Type | Description | Example |
|------|-------------|---------|
| **Scale** | Continuous numerical data | Age, Height, Income |
| **Ordinal** | Ordered categories | Education level (1=Low, 2=Medium, 3=High) |
| **Nominal** | Unordered categories | Gender, Color, Country |

**IMPORTANT:**
- JASP guesses variable types when loading data
- **It may guess wrong!**
- **You must check and fix variable types**

**Why it matters:**
- Wrong variable type → Wrong analysis options → Wrong results!

---

## Changing Variable Types

### How to Fix Variable Types

.pull-left[

**Two Methods:**

**Method 1: Click the Icon**
1. Click the symbol in the column header
2. Select the correct type from dropdown
3. Done!

**Method 2: Use the Variable Editor**
1. Double-click the column header
2. Change measurement level
3. Can also add labels here

]

.pull-right[

**Important Tips:**
- **IDs should be Nominal** (even if numbers)
- **Likert scales can be Ordinal or Scale** (your choice)
- **Binary variables (0/1) should be Nominal**
- **Age is Scale** (continuous numbers)

]



---

## `r icons::fontawesome("laptop")` HANDS-ON
### Loading the Titanic Dataset

.pull-left[

**Your Task:**

1. **Download** the file `titanic.csv` from the Canvas
2. **Open JASP** on your computer
3. **Load the data:**
   - File → Open → Computer → Browse
   - Find `titanic.csv` and open it

4. **Examine the data:**
   - How many rows (passengers)?
   - How many columns (variables)?
   - What variables do you see?

]

--

.pull-right[

**Expected:** 891 rows, 12 columns

]


---

## `r icons::fontawesome("laptop")` HANDS-ON
### Identifying & Fixing Variable Types

**The Titanic Dataset Variables:**

Look at your data and identify which type each variable should be:

.pull-left[

| Variable | Current Type? | Should Be? |
|----------|--------------|------------|
| PassengerId | ? | ? |
| Survived | ? | ? |
| Pclass | ? | ? |
| Name | ? | ? |
| Sex | ? | ? |
| Age | ? | ? |
| Fare | ? | ? |

]

.pull-right[

**Your Task:**
1. Check what type JASP assigned to each variable
2. Decide what type each should be
3. Fix any incorrect types by clicking the icon

]

---

## `r icons::fontawesome("laptop")` HANDS-ON - Solution

### Correct Variable Types for Titanic

| Variable | Should Be | Why? |
|----------|-----------|------|
| **PassengerId** | **Nominal** | ID number (not meaningful numerically) |
| **Survived** | **Nominal** | Binary: 0 = No, 1 = Yes |
| **Pclass** | **Nominal** or **Ordinal** | Ticket class: 1, 2, 3 (categories) |
| **Name** | **Nominal** | Text label (categorical) |
| **Sex** | **Nominal** | male or female (categorical) |
| **Age** | **Scale** | Continuous number (0-80 years) |
| **Fare** | **Scale** | Continuous number (price paid) |

**Key Decisions:**
- **Survived**: Could be ordinal, but typically treated as nominal for chi-square tests
- **Pclass**: Could be ordinal (ordered categories) or nominal—both work!

---

## Adding Value Labels in JASP

### Making Your Data More Readable

.pull-left[

**Why Add Labels?**
- Numeric codes (0, 1) are hard to interpret
- Labels make output easier to read
- Essential for clear reporting and presentations

**Example: Survived Variable**
- Current: 0 and 1 (confusing!)
- Better: "Did not survive" and "Survived" (clear!)

]

.pull-right[

**When to Use Labels:**
- Binary variables (Yes/No, Male/Female)
- Categorical codes (1=Low, 2=Medium, 3=High)
- Any nominal variable with numeric codes

]



---

##  `r icons::fontawesome("laptop")` HANDS-ON: Adding Labels to Survived

### Your Task: Make the Survived Variable More Interpretable

**Step-by-Step Instructions:**

.small[

.pull-left[

1. **Open the Variable Editor (Edit Data):**
   - Double-click on the **Survived** column header
   - OR right-click the column → **Edit column**

2. **Add Value Labels:**
   - In the variable editor window, find the **Labels** section
   - Click **+ Add label** 
   - For value **0**, enter label: **"Did not survive"**
   - Click **+ Add label** again
   - For value **1**, enter label: **"Survived"**

]

.pull-right[

3. **Apply and Check:**
   - Click outside the editor or press Enter
   - Run a frequency table: **Descriptives → Descriptive Statistics**
   - Move **Survived** to Variables
   - Check **Frequency tables**

4. **Observe the Difference:**
   - Your frequency table now shows meaningful labels!
   - Instead of "0" and "1", you see "Did not survive" and "Survived"

]

]

---

## What Are Descriptive Statistics?

### Summarising Your Data

**Descriptive statistics** help you understand your data by providing:

.pull-left[

**For Continuous Variables (Scale):**
- **Central tendency:** Where is the "middle"?
  - Mean, Median, Mode
- **Spread:** How spread out are the values?
  - Standard deviation, Variance, Range
  - Minimum, Maximum, Quartiles

**For Categorical Variables (Nominal/Ordinal):**
- **Frequencies:** How many in each category?
  - Counts and percentages

]

.pull-right[

**Why Use Them?**
- Understand your data before analysis
- Spot errors and outliers
- Describe your sample in reports

]



---

## Measures of Central Tendency

### Finding the "Middle" of Your Data

**Three Main Measures:**

.small[

.pull-left[

**1. Mean (Average)**
- Sum of all values ÷ number of values
- Most commonly used
- Sensitive to outliers
- Example: Ages = 20, 25, 30, 35, 100 → Mean = 42

**2. Median**
- Middle value when data is sorted
- Robust to outliers
- Example: Ages = 20, 25, **30**, 35, 100 → Median = 30

]

.pull-right[

**3. Mode**
- Most frequently occurring value
- Can be used for any variable type
- Example: Ages = 20, 25, 25, 25, 30 → Mode = 25

**Which to use?**
- **Normally distributed data** → Mean
- **Skewed data or outliers** → Median
- **Categorical data** → Mode

]

]

---

## Measures of Spread

### How Variable Is Your Data?

**Common Measures:**

.pull-left[

**1. Standard Deviation (SD)**
- Average distance from the mean
- Most commonly reported
- Same units as your data
- Larger SD = more spread out

**2. Variance**
- Squared standard deviation
- Less intuitive but used in calculations

]

.pull-right[

**3. Range**
- Maximum - Minimum
- Shows full spread

**4. Interquartile Range (IQR)**
- Range of middle 50% of data
- 75th percentile - 25th percentile
- Robust to outliers

]

---

## Computing Descriptives in JASP

### Step-by-Step Guide

.small[


**Steps:**

.pull-left[

1. **Open the Analysis Menu:**
   - Click **Descriptives** button (top menu bar)
   - Select **Descriptive Statistics**

2. **Select Variables:**
   - Drag variables from left box to **Variables** box
   - OR click variable and use arrow button
   
]
   
.pull-right[

3\. **Choose Statistics:**
   - Under **Statistics**, check boxes for what you want:
     - ☑ Mean
     - ☑ Median
     - ☑ Std. Deviation
     - ☑ Minimum / Maximum
     - etc.

4\. **View Results:**
   - Results appear immediately in right panel
   - Update in real-time as you change options

]


]

---

## `r icons::fontawesome("laptop")` HANDS-ON

### Descriptive Statistics for Age

**Your Task:**

Calculate descriptive statistics for the **Age** variable:

.pull-left[

1. Go to: **Descriptives** → **Descriptive Statistics**
2. Move **Age** to the **Variables** box
3. Under **Statistics**, select:
   - ☑ Mean
   - ☑ Median
   - ☑ Std. Deviation
   - ☑ Minimum
   - ☑ Maximum
   - ☑ Range
   
]
   
.pull-right[


**Questions to Answer:**
- What is the average age of Titanic passengers?
- What is the median age?
- What is the youngest age? Oldest?
- Is the mean or median higher? What does this tell us?

]

---

## `r icons::fontawesome("laptop")` HANDS-ON - DISCUSSION

### Interpreting Age Statistics

**Expected Results (approximately):**

.small[

.pull-left[


| Statistic | Value | Interpretation |
|-----------|-------|----------------|
| **Mean** | ~29.7 | Average age around 30 years |
| **Median** | ~28.0 | Middle age is 28 |
| **SD** | ~14.5 | Typical deviation is ±14-15 years |
| **Min** | ~0.42 | Youngest passenger was an infant |
| **Max** | ~80 | Oldest was 80 years old |
| **Range** | ~80 | Ages span 80 years |

]

.pull-right[

**Observations:**
- Mean > Median → Slight **positive skew** (right tail)
- Some older passengers pull the mean up
- Large SD indicates substantial age variation
- Note: Some ages are missing (Missing = 177)

]

]

---

## Frequency Tables for Categorical Data

### Counting Categories

.pull-left[

**For Nominal/Ordinal Variables:**

**Frequency tables show:**
- Count in each category
- Percentage in each category
- Cumulative percentages (for ordinal)

]

.pull-right[

**How to Create in JASP:**

1. **Descriptives** → **Descriptive Statistics**
2. Move categorical variable to **Variables** box
3. Under **Tables**, check:
   - ☑ **Frequency tables**

**Result:**
- Table showing counts and percentages
- Helps understand distribution of categories

]

---

## `r icons::fontawesome("laptop")` HANDS-ON
### Frequency Tables for Survival & Sex

.small[

.pull-left[

**Part A: Survival Status**

1. Create a frequency table for **Survived**
2. Answer:
   - How many passengers survived?
   - How many died?
   - What percentage survived?

**Part B: Sex Distribution**

3. Create a frequency table for **Sex**
4. Answer:
   - How many males vs females?
   - What percentage were male?

]

.pull-right[

**Part C: Passenger Class**

5. Create a frequency table for **Pclass**
6. Answer:
   - Which class had the most passengers?
   - What's the distribution across classes?

]

]


---

##  `r icons::fontawesome("laptop")` HANDS-ON - EXPECTED RESULTS

.small[

.pull-left[

**Survived:**

| Value | Count | Percent |
|-------|-------|---------|
| 0 (No) | 549 | 61.6% |
| 1 (Yes) | 342 | 38.4% |

**Sex:**

| Value | Count | Percent |
|-------|-------|---------|
| male | 577 | 64.8% |
| female | 314 | 35.2% |

**Pclass:**

| Value | Count | Percent |
|-------|-------|---------|
| 1 (First) | 216 | 24.2% |
| 2 (Second) | 184 | 20.7% |
| 3 (Third) | 491 | 55.1% |

]

.pull-right[

**Insights:**
- Most passengers died (61.6%)
- More males than females (about 2:1)
- Most passengers were in 3rd class (55%)

]

]

---

## `r icons::fontawesome("laptop")` HANDS-ON - Split Frequency Tables

### Understanding Survival Patterns Across Groups

**The Question:** Did survival rates differ by passenger class? By sex?

--

#### **Part A: Survival by Passenger Class (Follow Along)**

.small[

.pull-left[

**Steps:**
1. Go to: **Descriptives** → **Descriptive Statistics**
2. Move **Survived** to the **Variables** box
3. Move **Pclass** to the **Split** box
4. Under **Statistics**, check: ☑ **Frequency tables**

**What You See:**
- Separate frequency tables for each class (1st, 2nd, 3rd)
- Survival counts and percentages within each class

]

]

.pull-right[

**Observe:**
- Which class had the highest survival rate?
- Which class had the lowest?
- What pattern do you notice?

]

---

### `r icons::fontawesome("laptop")` HANDS-ON **Part B: Survival by Sex (Your Turn!)**

**Now You Try:**

Using the same approach:
1. Keep **Survived** in the **Variables** box
2. **Replace** Pclass with **Sex** in the **Split** box
3. Keep **Frequency tables** checked

**Questions to Answer:**
- What percentage of males survived?
- What percentage of females survived?
- Is there a notable difference?
- What might explain this pattern?

.content-box-yellow[**Discuss:** What story do these two analyses tell us about survival on the Titanic?]

---

## Visualising Distributions

### Adding Plots to Descriptives

**JASP makes it easy to visualize your data!**

**For Scale Variables:**

Under **Basic Plots/Customizable Plots** in Descriptive Statistics:
- ☑ **Distribution plots** → Histogram
- ☑ **Display density** → Smooth curve overlay
- ☑ **Boxplots** → Shows median, quartiles, outliers

**Benefits:**
- **Histograms:** See shape of distribution
- **Density plots:** Smooth distribution curve
- **Boxplots:** Quick summary with outliers

**For Categorical Variables:**
- Frequency tables are often sufficient
- Bar charts can be created elsewhere in JASP

---

## Slide 20: 🎯 IN-CLASS EXERCISE 5
### Visualizing Age Distribution

**Your Task:**

Create visualizations for the **Age** variable:

1. Keep Age in the **Variables** box
2. Under **Plots**, check:
   - ☑ **Distribution plots**
   - ☑ **Display density**
   - ☑ **Boxplots**

**Observe and Discuss:**
- What shape is the distribution?
- Is it symmetric or skewed?
- Where is the peak?
- Are there any outliers?
- What does the boxplot tell you about the median and quartiles?

**Take 3 minutes**

---

## Slide 21: Reading a Histogram

### Understanding Distribution Plots

**What to Look For:**

**Shape:**
- **Symmetric** (bell-shaped) → Normal distribution
- **Right skewed** (long right tail) → Most values on left
- **Left skewed** (long left tail) → Most values on right
- **Bimodal** (two peaks) → Two subgroups?

**Center:**
- Where is the peak (mode)?
- Is it where you expected?

**Spread:**
- How wide is the distribution?
- Are values clustered or dispersed?

**Age Distribution:**
- Slightly right skewed (mean > median)
- Peak around 20-30 years
- Reasonable spread across ages

---

## Slide 22: Reading a Boxplot

### Understanding Box-and-Whisker Plots

**Anatomy of a Boxplot:**

```
    ●  ← Outlier (beyond 1.5 × IQR)
    |
    ┬  ← Maximum (within 1.5 × IQR)
    |
    ┼──┐
    │  │
    │──│ ← Median (thick line)
    │  │
    └──┼
    |
    ┴  ← Minimum (within 1.5 × IQR)
    |
    ●  ← Outlier
```

**Box:** Contains middle 50% of data (25th to 75th percentile)
**Whiskers:** Extend to most extreme non-outlier values
**Dots:** Outliers beyond 1.5 × IQR

**Quick Assessment:**
- Symmetric box → Symmetric distribution
- Long upper whisker → Right skew
- Dots → Potential outliers to investigate

---

## Slide 23: Grouped Descriptive Statistics

### Comparing Groups

**What if we want to compare subgroups?**

**Example:** Age statistics separately for males and females

**How to Do It:**

1. **Descriptives** → **Descriptive Statistics**
2. Move continuous variable (Age) to **Variables**
3. Move grouping variable (Sex) to **Split** box
4. Select desired statistics

**Result:**
- Separate statistics for each group
- Easy comparison across groups
- Can also split plots!

**This is useful for:**
- Comparing survivors vs non-survivors
- Comparing passenger classes
- Any categorical breakdown

---

## Slide 24: 🎯 IN-CLASS EXERCISE 6
### Comparing Age by Survival Status

**Your Task:**

Compare age distributions for survivors vs non-survivors:

1. **Descriptives** → **Descriptive Statistics**
2. Move **Age** to **Variables**
3. Move **Survived** to **Split**
4. Check: Mean, Median, SD, Min, Max
5. Under **Plots**, check **Boxplots**

**Questions:**
- Were survivors younger or older on average?
- Is there a difference in median age?
- Do the boxplots show different patterns?
- What does this suggest?

**Take 4 minutes**

---

## Slide 25: 🎯 EXERCISE 6 - DISCUSSION

### Age by Survival Status

**Expected Pattern:**

| Survived | Mean Age | Median Age | Observations |
|----------|----------|------------|--------------|
| 0 (Died) | ~30.6 | 28 | Slightly older |
| 1 (Survived) | ~28.3 | 28 | Slightly younger |

**Insights:**
- Small difference in mean age
- Similar median ages
- Children may have had better survival rates
- But age alone doesn't tell the whole story!

**Next Steps:**
- These are just descriptive statistics
- To test if differences are **significant**, we need:
  - Chi-square test (for categorical comparisons)
  - t-test (for mean comparisons)
  - More complex analyses

**This is where hypothesis testing comes in!**

---

## Slide 26: Dealing with Missing Data

### What About Missing Values?

**JASP Handles Missing Data Automatically:**

**In the Data:**
- Missing values shown as blank cells
- Or sometimes as "." or "NA"

**In Descriptives:**
- "Missing" count shows how many are missing
- "Valid" count shows how many have data
- Statistics calculated only on valid cases

**Age Variable:**
- 177 passengers have missing ages
- Valid N = 714
- About 20% missing

**Best Practices:**
- Always report missing data
- Check if missing data is random or systematic
- Consider implications for your analysis

---

## Slide 27: 🎯 IN-CLASS EXERCISE 7
### Complete Descriptive Analysis

**Your Challenge:**

Perform a complete descriptive analysis of **Fare**:

1. Calculate descriptive statistics:
   - Central tendency (Mean, Median)
   - Spread (SD, Range, IQR)
   - Check for missing values

2. Create visualizations:
   - Histogram with density
   - Boxplot

3. Compare across passenger classes:
   - Split by **Pclass**
   - Compare mean fares

4. Interpret:
   - What is the typical fare?
   - How variable are the fares?
   - Do fares differ by class?

**Take 5-7 minutes**

---

## Slide 28: 🎯 EXERCISE 7 - DISCUSSION

### Fare Analysis Insights

**Expected Findings:**

**Overall Statistics:**
- Mean fare: ~£32 (but highly skewed!)
- Median fare: ~£14 (more representative)
- Huge SD and range (some very expensive tickets)
- Several high-value outliers

**By Passenger Class:**
| Class | Mean Fare | Median Fare |
|-------|-----------|-------------|
| 1st | ~£84 | ~£60 |
| 2nd | ~£21 | ~£14 |
| 3rd | ~£13 | ~£8 |

**Observations:**
- Fare is heavily right-skewed
- Median better than mean (outliers present)
- Clear class differences in ticket prices
- 1st class passengers paid 6x more on average!

---

## Slide 29: Exporting Your Results

### Saving Your Work

**Three Main Options:**

**1. Copy Individual Tables/Plots:**
- Right-click on any result
- Select **Copy** or **Copy Special**
- Paste into Word, PowerPoint, etc.
- Tables are APA-formatted!

**2. Export Data:**
- **File** → **Export Data**
- Save as CSV (can open in Excel)
- Useful if you've computed new variables

**3. Save JASP File:**
- **File** → **Save As**
- Saves data AND all analyses
- Can reopen later and continue work
- Share with collaborators

**Pro Tip:** Save as .jasp file regularly to preserve your work!

---

## Slide 30: Best Practices for Descriptive Statistics

### Tips for Success

✅ **DO:**
- Always check variable types first!
- Calculate descriptives before formal tests
- Visualize your data (plots reveal patterns)
- Report both central tendency and spread
- Note missing data
- Use median for skewed distributions
- Compare groups when relevant

❌ **DON'T:**
- Trust JASP's automatic variable type guessing
- Report only mean without SD
- Ignore outliers without investigation
- Forget to check for missing data
- Over-interpret small differences
- Use mean for highly skewed data

**Remember:** Descriptive statistics are the foundation of all analyses!

---

## Slide 31: Summary - What We Learned

### Key Takeaways

**Getting Started with JASP:**
✓ Installing and launching JASP
✓ Loading CSV files
✓ Understanding the interface

**Variable Types:**
✓ Scale (continuous), Ordinal (ordered), Nominal (categorical)
✓ How to check and change types
✓ Why variable types matter

**Descriptive Statistics:**
✓ Measures of central tendency (mean, median, mode)
✓ Measures of spread (SD, range, IQR)
✓ Frequency tables for categorical data
✓ Visualizations (histograms, boxplots)
✓ Split analysis by groups

**Titanic Dataset:**
✓ Practiced with real data
✓ Explored passenger characteristics
✓ Compared groups (survivors, classes, sex)

---

## Slide 32: Looking Ahead

### Next Steps in Your JASP Journey

**You now have the foundation to:**

1. **Explore your own data**
   - Load your datasets
   - Calculate descriptive statistics
   - Create informative visualizations

2. **Move to inferential statistics:**
   - Hypothesis testing (is there a real difference?)
   - Chi-square tests (categorical relationships)
   - t-tests (comparing means)
   - Correlation and regression (relationships)

3. **Build more complex analyses:**
   - ANOVA (comparing multiple groups)
   - Multiple regression (many predictors)
   - Advanced visualizations

**Practice is key!** The more you use JASP, the more comfortable you'll become.

---

## Slide 33: Quick Reference Card

### Essential JASP Commands

| Task | Location | Key Options |
|------|----------|-------------|
| **Load data** | File → Open | Choose .csv or .jasp |
| **Change variable type** | Click column icon | Scale / Ordinal / Nominal |
| **Descriptive stats** | Descriptives → Descriptive Statistics | Check desired statistics |
| **Frequency tables** | Same location | Check "Frequency tables" |
| **Histograms** | Under Plots | Check "Distribution plots" |
| **Boxplots** | Under Plots | Check "Boxplots" |
| **Group comparisons** | Move grouping var to Split | Separate stats per group |
| **Copy results** | Right-click result | Copy or Copy Special |
| **Save work** | File → Save As | Save as .jasp file |

**Bookmark this page for quick access!**

---

## Slide 34: Practice Problems

### Take Home Exercises

**Challenge Yourself:**

1. **Explore Fare Distribution**
   - Calculate coefficient of variation (SD/Mean × 100%)
   - Compare fare distributions across all three classes with boxplots
   - Identify outliers and investigate them in the data

2. **Survival Analysis**
   - Create frequency tables for Survived × Sex (cross-tabulation)
   - Create frequency tables for Survived × Pclass
   - Calculate survival rates for each group

3. **Age Groups**
   - Create age categories (Child <18, Adult 18-60, Senior >60)
   - Compare survival rates across age groups
   - Compare fare by age groups

**Hint:** Some of these require skills we'll learn in later sessions!

---

## Slide 35: Common Mistakes & Troubleshooting

### When Things Go Wrong

**Problem:** Variable won't show up in analysis options
- **Solution:** Check variable type (might need to be Scale/Nominal)

**Problem:** "Not enough observations" error
- **Solution:** Too much missing data or wrong filter applied

**Problem:** Results look strange or unexpected
- **Solution:** Check variable types, check for outliers, verify data loaded correctly

**Problem:** Can't find a specific analysis option
- **Solution:** Variable might be wrong type for that analysis

**Problem:** Plots won't display
- **Solution:** Make sure variable is correct type for that plot type

**Getting Help:**
- JASP documentation: https://jasp-stats.org/
- JASP forums and YouTube tutorials
- Ask your instructor!

---

## Slide 36: Additional Resources

### Keep Learning!

**Official Resources:**
- **JASP Website:** https://jasp-stats.org/
- **JASP Documentation:** Comprehensive guides and tutorials
- **YouTube:** Search "JASP statistics tutorial"
- **JASP Forum:** Community support

**Textbook:**
- *Learning Statistics with JASP* (the source for today's material)
- Free online: https://learnstatswithjasp.com/

**Practice Datasets:**
- JASP includes built-in example datasets
- Many free datasets available online
- UCI Machine Learning Repository
- Kaggle datasets

**Stay Curious:**
- Explore JASP's advanced features
- Try different types of plots
- Experiment with your own data

---

## Slide 37: Questions & Discussion

### Let's Review

**Key Questions to Consider:**

1. What surprised you about the Titanic data?
2. Which descriptive statistic do you find most useful?
3. When would you use median instead of mean?
4. How do visualizations help you understand data?
5. What analyses would you like to run on this dataset next?

**Open Discussion:**
- Share your observations from the exercises
- Discuss any challenges you encountered
- Ask questions about anything unclear

**Remember:** There are no silly questions in statistics!

---

## Slide 38: Class Activity - Group Work

### Collaborative Analysis Challenge

**Form groups of 3-4 students**

**Your Mission:**
Prepare a brief descriptive summary of the Titanic dataset to present to the class. Include:

1. **Sample Description:**
   - Total passengers
   - Key demographic information (age, sex distribution)
   - Passenger class breakdown

2. **Survival Overview:**
   - Overall survival rate
   - One interesting pattern you discovered

3. **One Visualization:**
   - Create one compelling plot
   - Be ready to explain what it shows

**Time:** 10 minutes to analyze, 2 minutes to present

**This simulates a real research scenario!**

---

## Slide 39: Wrap-Up & Next Session Preview

### Today's Accomplishments

**You Successfully Learned:**
✓ How to install and navigate JASP
✓ How to load and inspect data
✓ How to identify and change variable types
✓ How to calculate descriptive statistics
✓ How to create informative visualizations
✓ How to compare groups using splits

**Next Session Preview:**
- Hypothesis testing concepts
- Chi-square tests (categorical data)
- Comparing survival rates statistically
- Testing for significant relationships
- Introduction to t-tests

**Homework:** Practice with the Titanic dataset and bring questions!

---

## Slide 40: Thank You!

### Stay Curious, Keep Analyzing!

**Contact Information:**
[Your email/office hours]

**Course Materials:**
[Link to datasets and resources]

**Remember:**
- **JASP makes statistics accessible**
- **Start with descriptive statistics**
- **Visualize before you analyze**
- **Practice regularly**

### Questions? Feedback? Let's discuss!

**Good luck with your statistical journey!** 📊✨

---

# Appendix: Titanic Variable Codebook

## Complete Variable Descriptions

| Variable | Type | Description | Values |
|----------|------|-------------|--------|
| PassengerId | Nominal | Unique ID for each passenger | 1-891 |
| Survived | Nominal | Survival status | 0 = No, 1 = Yes |
| Pclass | Nominal/Ordinal | Ticket class (socioeconomic status) | 1 = 1st, 2 = 2nd, 3 = 3rd |
| Name | Nominal | Passenger name | Text |
| Sex | Nominal | Gender | male, female |
| Age | Scale | Age in years | 0.42-80 years |
| SibSp | Scale/Nominal | # of siblings/spouses aboard | 0-8 |
| Parch | Scale/Nominal | # of parents/children aboard | 0-6 |
| Ticket | Nominal | Ticket number | Various |
| Fare | Scale | Passenger fare in pounds | 0-512 |
| Cabin | Nominal | Cabin number | Various (many missing) |
| Embarked | Nominal | Port of embarkation | C=Cherbourg, Q=Queenstown, S=Southampton |

---

# End of Slides


